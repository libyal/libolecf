...
/* Copies the path from a comparable item path
 * Returns 1 if successful or -1 on error
 */
int mount_path_string_copy_from_item_path(
     system_character_t **path,
     size_t *path_size,
     const system_character_t *item_path,
     size_t item_path_length,
     libcerror_error_t **error )
{
	system_character_t *safe_path                = NULL;
	static char *function                        = "mount_path_string_copy_from_item_path";
	...
	if( ( item_path_length == 0 )
	 || ( item_path_length > (size_t) ( SSIZE_MAX - 1 ) ) )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_ARGUMENTS,
		 LIBCERROR_ARGUMENT_ERROR_VALUE_OUT_OF_BOUNDS,
		 "%s: invalid item path length value out of bounds.",
		 function );
	 ...
	 */
	if( item_path_length > (size_t) ( ( SSIZE_MAX - 1 ) / ( sizeof( system_character_t ) * 10 ) ) )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_ARGUMENTS,
	...
	safe_path_size = ( item_path_length * 10 ) + 1;
	...
	while( item_path_index < item_path_length )
	{
#if defined( HAVE_WIDE_SYSTEM_CHARACTER )
		result = libuna_unicode_character_copy_from_utf16(
		          &unicode_character,
		          (libuna_utf16_character_t *) item_path,
		          item_path_length,
		          &item_path_index,
		          error );
#else
		result = libuna_unicode_character_copy_from_utf8(
		          &unicode_character,
		          (libuna_utf8_character_t *) item_path,
		          item_path_length,
		          &item_path_index,
		          error );
#endif
		if( result != 1 )
		{
			libcerror_error_set(
			 ...
			 "%s: unable to copy Unicode character from item path.",
			 function );
...
/* Copies the path to a comparable item path
 * Returns 1 if successful or -1 on error
 */
int mount_path_string_copy_to_item_path(
     ...
     libcerror_error_t **error )
{
	...
	static char *function                        = "mount_path_string_copy_to_item_path";
	libuna_unicode_character_t unicode_character = 0;
	...
	while( path_index < path_length )
	{
		...
		{
			unicode_character = (libuna_unicode_character_t) LIBOLECF_SEPARATOR;
		}
		else if( unicode_character == (libuna_unicode_character_t) ESCAPE_CHARACTER )
		{
