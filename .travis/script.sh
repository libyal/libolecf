...
then
	docker run -t -v "${PWD}:/libolecf" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libolecf && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libolecf;
