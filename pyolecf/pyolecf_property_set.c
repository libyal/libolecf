      ...
      pyolecf_property_set_t *pyolecf_property_set )
{
	...
	if( pyolecf_property_set->ob_type == NULL )
	{
		PyErr_Format(
		 PyExc_TypeError,
		 "%s: invalid property set - missing ob_type.",
		 function );

		return;
	}
	if( pyolecf_property_set->ob_type->tp_free == NULL )
	{
		PyErr_Format(
		 PyExc_TypeError,
		 "%s: invalid property set - invalid ob_type - missing tp_free.",
		 function );
	...
	pyolecf_property_set->ob_type->tp_free(
	 (PyObject*) pyolecf_property_set );
}
