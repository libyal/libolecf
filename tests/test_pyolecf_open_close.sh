...
{ 
	...
	then
		DYLD_LIBRARY_PATH="../libolecf/.libs/" PYTHONPATH="../pyolecf/.libs/" ${PYTHON} ${SCRIPT} ${INPUT_FILE};
		RESULT=$?;
	else
		LD_LIBRARY_PATH="../libolecf/.libs/" PYTHONPATH="../pyolecf/.libs/" ${PYTHON} ${SCRIPT} ${INPUT_FILE};
		RESULT=$?;
	fi
...
SCRIPT="pyolecf_test_open_close.py";
