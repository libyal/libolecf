		...
		do
			...
			do
				...
				then
					...
					run_test_on_input_file "${TEST_SET_DIRECTORY}" "olecfexport" "with_callback" "${OPTION_SET}" "${TEST_EXECUTABLE}" "${INPUT_FILE}" "${PROFILE_OPTIONS[@]}" "${OPTIONS[@]}";
					RESULT=$?;
			...
			then
				run_test_on_input_file "${TEST_SET_DIRECTORY}" "olecfexport" "with_callback" "" "${TEST_EXECUTABLE}" "${INPUT_FILE}" "${PROFILE_OPTIONS[@]}";
				RESULT=$?;
			fi
